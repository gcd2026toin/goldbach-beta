...
- **3段階のボット** — 初級・中級・上級。ホーム画面でタップするたびに切り替えられ、設定はセット間を通じて保持されます。
- **トロフィーシステム** — 数論的に特徴的なあがり方（互いに素な3枚組・公約数出し・素数テーブルなど）に対応した実績が13種類あります。
- **ダークモード** — デバイスのシステム設定に自動で追従します。一部のブラウザでは意図しない色合いになります。→「対応ブラウザ」項目
- **効果音** — カード操作に合わせて再生されます。
...
Google Chrome, Safari, Saumsung Browser, Microsoft Edge


ただしSamsung Browserのみ、ダークモードにおいて一部のボタンが深い赤色を呈し視認性が悪くなります。現在修正を検討中。
